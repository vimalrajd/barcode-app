    ...
    (async () => {
      ...
      for (const item of items) {
        ...
        for (let copy = 0; copy < item.quantity; copy++) {
          ...
          doc.font('Helvetica-Bold').fontSize(14).fillColor('#3d3d3d');
          doc.text(priceText, margin, y, { width: contentWidth, height: 15, align: 'center', ellipsis: true });
          y += 16;

          const barcodeDrawWidth = contentWidth - 10;
          const barcodeDrawHeight = 21;
          doc.image(barcodePng, margin + 5, y, { width: barcodeDrawWidth, height: barcodeDrawHeight });
          y += barcodeDrawHeight + 2;

          doc.moveTo(margin, y).lineTo(LABEL_WIDTH - margin, y).strokeColor('#999999').lineWidth(0.4).stroke();
          y += 2;

          doc.font('Helvetica-Bold').fontSize(12).fillColor('#3d3d3d');
          doc.text(item.sku, margin, y, { width: contentWidth, height: 13, align: 'center', ellipsis: true });
          y += 13;

          if (companyLine) {
            doc.font('Helvetica').fontSize(5).fillColor('#3d3d3d');
