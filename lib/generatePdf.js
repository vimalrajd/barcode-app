...
async function makeBarcodePng(text) {
  return bwipjs.toBuffer({
    bcid: 'code128',
    text: String(text),
    scale: 6, // high source resolution so the barcode stays crisp when printed
    height: 9,
    includetext: false,
    ...
    (async () => {
      ...
      for (const item of items) {
        ...
        for (let copy = 0; copy < item.quantity; copy++) {
          ...
          doc.font('Helvetica-Bold').fontSize(8).fillColor('#3d3d3d');
          doc.text(priceText, margin, y, { width: contentWidth, height: 9, align: 'center', ellipsis: true });
          y += 9.5;

          const barcodeDrawWidth = contentWidth - 6;
          const barcodeDrawHeight = 10;
          doc.image(barcodePng, margin + 3, y, { width: barcodeDrawWidth, height: barcodeDrawHeight });
          y += barcodeDrawHeight + 1;
          ...
          doc.text(item.sku, margin, y, { width: contentWidth, height: 10.5, align: 'center', ellipsis: true });
          y += 10.8;

          if (companyLine) {
            doc.font('Helvetica').fontSize(6).fillColor('#3d3d3d');
            const remaining = LABEL_HEIGHT - y - margin;
